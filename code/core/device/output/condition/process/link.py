...
def get_link_result(result_dict: dict, link, device: str) -> bool:
    ...
    if len(result_dict) != 2 or LINK_ORDER_ID_1 not in result_dict or LINK_ORDER_ID_2 not in result_dict:
        ...
        raise ValueError("Got more/less than two results \"%s\" for link \"%s\" (id \"%s\"); Could be a configuration error"
                         % (result_dict, link.name, link.object_id))
    ...
    if op == 'and':
        ...
        raise KeyError("Link \"%s\" (id \"%s\") has an unsupported operator \"%s\"" % (link.name, link.object_id, link.operator))
